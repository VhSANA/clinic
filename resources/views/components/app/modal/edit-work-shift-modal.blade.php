...
<button
    id="open-edit-modal-btn-{{$schedule_date}}"
...
<div id="edit-personnel-shift-{{$schedule_date}}" class="{{ ($errors->any() && session("edit_modal_open_$schedule_date")) ? 'flex' : 'hidden' }} bg-gray-500 bg-opacity-40 overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-7xl max-h-full">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                ...
                <button type="button" id="close-edit-modal-btn-{{$schedule_date}}" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm h-8 w-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white">
            ...
            <div class="p-4 md:p-5">
                <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
                    <form class="w-full " action="{{ route('schedule.update', $schedule->id) }}" method="POST">
                        ...
                        {{-- عنوان شیفت --}}
                        <x-app.input.edit-inputs name="title_{{ $schedule_date }}" label="عنوان شیفت:*" value="{{$schedule->title}}" />
                        ...
                        <!-- انتاخب خدمت -->
                        <x-app.input.edit-inputs name="service_{{$schedule_date}}" type="select" label="انتخاب خدمت درمانی*" >
                            @foreach (App\Models\Personnel::find($personnel)->medicalservices as $service)
                                <option value="{{ $service->id }}" {{ old('service', App\Models\MedicalServices::find($schedule->medical_service_id)) == $service->id ? 'selected' : '' }}>
                                    {{ $service->name }}
                                </option>
                            @endforeach
                        ...
                        <!-- انتخاب اتاق -->
                        <x-app.input.edit-inputs name="room_{{$schedule_date}}" type="select" label="انتخاب اتاق*" >
                            @foreach ($rooms as $room)
                                <option value="{{ $room->id }}" {{ old('room', App\Models\Room::find($schedule->room_id)->id ) == $room->id ? 'selected' : '' }}>
                                    {{ $room->title }}
                                </option>
                            @endforeach
                        </x-app.input.edit-inputs>
                        ...
                        <div class="flex justify-evenly mt-3">
                            ...
                            <button type="button" id="cancel-edit-modal-btn_{{$schedule_date}}" class="rounded-full  bg-red-600 dark:bg-red-800 text-white dark:text-white antialiased font-bold hover:bg-red-800 dark:hover:bg-red-900 px-4 py-2 flex items-center justify-between transition">لغو <x-cancel-icon /></button>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const modal = document.getElementById(`edit-personnel-shift-${scheduleDate}`);
        const openModal = document.getElementById(`open-edit-modal-btn-${scheduleDate}`);
        const closeModal = document.getElementById(`close-edit-modal-btn-${scheduleDate}`);
        const cancelModal = document.getElementById(`cancel-edit-modal-btn-${scheduleDate}`);
